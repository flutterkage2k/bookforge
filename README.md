...
5. **빌드·검사** — 빌드 → 게이트 → 지면 이미지. 실패하면 **한국어로** 원인과 할 일을 표로 보여주고,
   각 실패가 **누구 문제인지**(내 원고 / 스타일 설정 / 도구 결함) 표시합니다. 「AI에게 맡기기」가
   손댈 수 있는 코드인지도 알려주므로, 안 되는 걸 계속 누르고 있을 일이 없습니다.
   고칠 수 없는 실패면 「오류 신고」가 재현에 필요한 것만 묶어 보여줍니다(원고 전문 제외)
...
웹 UI 5단계에서 「오류 신고」를 누르면 **보낼 내용을 먼저 보여줍니다.** 확인한 뒤 고르세요.

- **GitHub 이슈** — [issues](https://github.com/flutterkage2k/bookforge/issues). 중복·검색·이력이 남습니다
- **메일** — flutterkage2k@gmail.com. 원고를 공개하기 곤란할 때

신고에 들어가는 것: 게이트의 판정 근거, 게이트가 **지목한 원고 부분만**, 스타일, 도구 버전.
원고 전문은 들어가지 않습니다. 그래도 이슈는 공개이니 내용을 보고 판단하세요.

텔레그램·슬랙·디스코드는 쓰지 않습니다. 셋 다 보내는 쪽에 봇 토큰이나 웹훅 URL이
있어야 하는데, 이 서버의 소스는 공개 저장소에 있어 토큰을 넣는 순간 누구나 그 채널에
쏠 수 있게 됩니다.
