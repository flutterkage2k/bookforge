...
4. **집필** — 장별 편집, 또는 「AI에게 전체 집필 맡기기」·「AI에게 도해 넣기」.
   사진·스크린샷은 「이미지 넣기」 버튼으로 올리거나 **복사해서 본문에 Cmd+V** —
   폭은 절반·2/3·전폭 중 선택(기본 절반 — 세로로 긴 스크린샷이 전폭이면 한 면을 다 먹습니다).
   책 폴더 `assets/`에 저장되고 커서 자리에 `![캡션](../assets/…)` 링크가 끼워집니다.
   캡션과 출처를 채우고 저장하면 빌드 때 `그림 n-m` 번호가 자동으로 붙습니다.
   원격 URL은 받지 않습니다 — 링크가 죽으면 책이 빌드 불가가 되고, 라이선스 확인도 안 되기 때문입니다
...
**스타일을 바꾸고 싶으면** — 1단계 책 정보의 「다른 스타일로 복사본 만들기」.
원고·자료·도해 소스만 복사되고 원본은 변하지 않습니다. 복사본에서 도해가 거부되면
(색 팔레트·글자 하한이 스타일마다 다릅니다) 4단계 「AI에게 도해 넣기」로 다시 생성하세요.
